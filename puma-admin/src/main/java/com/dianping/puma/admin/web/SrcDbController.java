...
import com.dianping.puma.admin.model.mapper.SrcDbMapper;
...
public class SrcDbController {
	...
	private static final Logger logger = LoggerFactory.getLogger(SrcDbController.class);
	...
	public String list(int page, int pageSize) {
		...
		long count = srcDbService.count();
		map.put("count", count);

		List<SrcDbEntity> srcDbEntities = srcDbService.findAll();
		List<SrcDbDto> srcDbDtos = SrcDbMapper.map(srcDbEntities);
		map.put("list", srcDbDtos);
