 ...
 *
 */
public class DateTime2TypeTest extends AbstractBaseTest {

	private static final Logger LOG = LoggerFactory.getLogger(DateTime2TypeTest.class);
		...
		test(new TestLogic() {
			...
			public void doLogic() throws Exception {
				...
				for (int i = 0; i < testData.length; i++) {
					...
					Assert.assertEquals(testData[i][0],String.valueOf(rowChangedEvent.getColumns().get("size_dateTime1").getNewValue()));
					Assert.assertEquals(null, rowChangedEvent.getColumns().get("size_dateTime1").getOldValue());
					Assert.assertEquals(testData[i][1],String.valueOf(rowChangedEvent.getColumns().get("size_dateTime2").getNewValue()));
					Assert.assertEquals(null, rowChangedEvent.getColumns().get("size_dateTime2").getOldValue());
					Assert.assertEquals(testData[i][2],String.valueOf(rowChangedEvent.getColumns().get("size_dateTime3").getNewValue()));
					Assert.assertEquals(null, rowChangedEvent.getColumns().get("size_dateTime3").getOldValue());
					Assert.assertEquals(testData[i][3],String.valueOf(rowChangedEvent.getColumns().get("size_dateTime4").getNewValue()));
					Assert.assertEquals(null, rowChangedEvent.getColumns().get("size_dateTime4").getOldValue());
					Assert.assertEquals(testData[i][4],String.valueOf(rowChangedEvent.getColumns().get("size_dateTime5").getNewValue()));
					Assert.assertEquals(null, rowChangedEvent.getColumns().get("size_dateTime5").getOldValue());
					Assert.assertEquals(testData[i][5],String.valueOf(rowChangedEvent.getColumns().get("size_dateTime6").getNewValue()));
					Assert.assertEquals(null, rowChangedEvent.getColumns().get("size_dateTime6").getOldValue());
					Assert.assertEquals(testData[i][6],String.valueOf(rowChangedEvent.getColumns().get("unsize_dateTime").getNewValue()));
