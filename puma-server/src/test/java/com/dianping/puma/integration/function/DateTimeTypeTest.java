...
public class DateTimeTypeTest extends AbstractBaseTest {
	
	private static final Logger LOG = LoggerFactory.getLogger(DateTimeTypeTest.class);
		...
		test(new TestLogic() {
			...
			public void doLogic() throws Exception {
				...
				for(int i = 0; i < testData.length; i++){
					...
					Assert.assertEquals(testData[i][0], String.valueOf(rowChangedEvent.getColumns().get("default_dateTime").getNewValue()));
