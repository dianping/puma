 ...
 *
 */
public class MediumBlobTypeTest extends AbstractBaseTest {
	
	private static final Logger LOG = LoggerFactory.getLogger(MediumBlobTypeTest.class);
	
	private static final String TABLE_NAME = "tb_mediumBlob";
		...
		test(new TestLogic() {
			...
			public void doLogic() throws Exception {
				...
				for(int i = 0; i < testData.length; i++){
					String insert_SQL = "INSERT INTO `" + SCHEMA_NAME +"`.`" + TABLE_NAME + "`(default_mediumBlob)VALUES(?)";
				...
				for(int i = 0; i < testData.length; i++){
					...
					Assert.assertEquals(testData[i][0], new String((byte[])rowChangedEvent.getColumns().get("default_mediumBlob").getNewValue(), "UTF-8"));
