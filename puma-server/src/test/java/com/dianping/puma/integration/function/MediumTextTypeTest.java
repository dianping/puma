...
/***
 * mediumtext type test
 * @author qi.yin
 *
 */
public class MediumTextTypeTest extends AbstractBaseTest {
	
	private static final Logger LOG = LoggerFactory.getLogger(MediumTextTypeTest.class);
	
	private static final String TABLE_NAME = "tb_mediumText";
	
	@BeforeClass
	public static void doBefore() throws Exception {
		String create_SQL = "CREATE TABLE IF NOT EXISTS `" + SCHEMA_NAME +"`.`" + TABLE_NAME + "` (\n"
				+ "`id` int NOT NULL AUTO_INCREMENT, \n" + "`binary_mediumText` mediumtext BINARY DEFAULT NULL, \n"
				+ "`unbinary_mediumText` mediumtext DEFAULT NULL, \n" 
				+ "PRIMARY KEY (`id`)"
	...
	@Test
	public void mediumTextTypeInsertTest() throws Exception {
		test(new TestLogic() {
			...
			public void doLogic() throws Exception {
				...
				for(int i = 0; i < testData.length; i++){
					String insert_SQL = "INSERT INTO `" + SCHEMA_NAME +"`.`" + TABLE_NAME + "`(binary_mediumText, unbinary_mediumText)VALUES(?, ?)";
				...
				for(int i = 0; i < testData.length; i++){
					...
					Assert.assertEquals(testData[i][0], new String((byte[])rowChangedEvent.getColumns().get("binary_mediumText").getNewValue(), "UTF-8"));
					Assert.assertEquals(null, rowChangedEvent.getColumns().get("binary_mediumText").getOldValue());
					Assert.assertEquals(testData[i][1], new String((byte[])rowChangedEvent.getColumns().get("unbinary_mediumText").getNewValue(), "UTF-8"));
					Assert.assertEquals(null, rowChangedEvent.getColumns().get("unbinary_mediumText").getOldValue());
				}
			}
