			...
			public void doLogic() throws Exception {
				...
				for (int i = 0; i < testData.length; i++) {
					String insert_SQL = "INSERT INTO `" + SCHEMA_NAME + "`.`" + TABLE_NAME
							+ "`(binary_mediumText, unbinary_mediumText)VALUES(?, ?)";
				...
				for (int i = 0; i < testData.length; i++) {
					...
					Assert.assertEquals(testData[i][0],
							new String((byte[]) rowChangedEvent.getColumns().get("binary_mediumText").getNewValue(),
									"UTF-8"));
					Assert.assertEquals(null, rowChangedEvent.getColumns().get("binary_mediumText").getOldValue());
					Assert.assertEquals(testData[i][1],
							new String((byte[]) rowChangedEvent.getColumns().get("unbinary_mediumText").getNewValue(),
									"UTF-8"));
