	...
	public void textTypeInsertTest() throws Exception {
		test(new TestLogic() {
			...
			public void doLogic() throws Exception {
				String [][] testData = {{"ccc", "ccc", "ccc", "ccc"},{"ccccc", "ccccc", "ccccc", "ccccc"},{"cccccc", "cccccc", "cccccc", "cccccc"}};
				for(int i = 0; i < testData.length; i++){
					String insert_SQL = "INSERT INTO `" + SCHEMA_NAME +"`.`" + TABLE_NAME + "`(binary_text, unbinary_text, size_text, unsize_text)VALUES(?, ?, ?, ?)";
				...
				for(int i = 0; i < testData.length; i++){
					...
					Assert.assertEquals(testData[i][0], new String((byte[])rowChangedEvent.getColumns().get("binary_text").getNewValue(), "UTF-8"));
					Assert.assertEquals(null, rowChangedEvent.getColumns().get("binary_text").getOldValue());
					Assert.assertEquals(testData[i][1], new String((byte[])rowChangedEvent.getColumns().get("unbinary_text").getNewValue(), "UTF-8"));
					Assert.assertEquals(null, rowChangedEvent.getColumns().get("unbinary_text").getOldValue());
					Assert.assertEquals(testData[i][2], new String((byte[])rowChangedEvent.getColumns().get("size_text").getNewValue(), "UTF-8"));
					Assert.assertEquals(null, rowChangedEvent.getColumns().get("size_text").getOldValue());
					Assert.assertEquals(testData[i][3], new String((byte[])rowChangedEvent.getColumns().get("unsize_text").getNewValue(), "UTF-8"));
