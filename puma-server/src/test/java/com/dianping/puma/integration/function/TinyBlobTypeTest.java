 ...
 *
 */
public class TinyBlobTypeTest extends AbstractBaseTest {
	
	private static final Logger LOG = LoggerFactory.getLogger(TinyBlobTypeTest.class);
	...
	public static void doBefore() throws Exception {
		String create_SQL = "CREATE TABLE IF NOT EXISTS `" + SCHEMA_NAME +"`.`" + TABLE_NAME + "` (\n"
				+ "`id` int NOT NULL AUTO_INCREMENT, \n" + "`default_tinyBlob` tinyblob DEFAULT NULL, \n"
				+ "PRIMARY KEY (`id`)"
	...
	@Test
	public void tinyBlobTypeInsertTest() throws Exception {
		test(new TestLogic() {
			...
			public void doLogic() throws Exception {
				...
				for(int i = 0; i < testData.length; i++){
					String insert_SQL = "INSERT INTO `" + SCHEMA_NAME +"`.`" + TABLE_NAME + "`(default_tinyBlob)VALUES(?)";
				...
				for(int i = 0; i < testData.length; i++){
					...
					Assert.assertEquals(testData[i][0], new String((byte[])rowChangedEvent.getColumns().get("default_tinyBlob").getNewValue(), "UTF-8"));
					Assert.assertEquals(null, rowChangedEvent.getColumns().get("default_tinyBlob").getOldValue());
				}
			}
