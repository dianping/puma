			...
			public void doLogic() throws Exception {
				...
				for (int i = 0; i < testData.length; i++) {
					...
					Assert.assertEquals(testData[i][0],
							new String((byte[]) rowChangedEvent.getColumns().get("binary_tinyText").getNewValue(),
									"UTF-8"));
					Assert.assertEquals(null, rowChangedEvent.getColumns().get("binary_tinyText").getOldValue());
					Assert.assertEquals(testData[i][1],
							new String((byte[]) rowChangedEvent.getColumns().get("unbinary_tinyText").getNewValue(),
									"UTF-8"));
