		...
		test(new TestLogic() {
			...
			public void doLogic() throws Exception {
				...
				for(int i = 0; i < testData.length; i++){
					String insert_SQL = "INSERT INTO `" + SCHEMA_NAME +"`.`" + TABLE_NAME + "`(binary_varchar, unbinary_varchar)VALUES(?, ?)";
				...
				for(int i = 0; i < testData.length; i++){
					...
					Assert.assertEquals(testData[i][0], String.valueOf(rowChangedEvent.getColumns().get("binary_varchar").getNewValue()));
					Assert.assertEquals(null, rowChangedEvent.getColumns().get("binary_varchar").getOldValue());
					Assert.assertEquals(testData[i][1], String.valueOf(rowChangedEvent.getColumns().get("unbinary_varchar").getNewValue()));
