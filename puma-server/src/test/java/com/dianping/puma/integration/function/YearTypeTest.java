 ...
 *
 */
public class YearTypeTest extends AbstractBaseTest {
	
	private static final Logger LOG = LoggerFactory.getLogger(YearTypeTest.class);
		...
		test(new TestLogic() {
			...
			public void doLogic() throws Exception {
				...
				for(int i = 0; i < testData.length; i++){
					String insert_SQL = "INSERT INTO `" + SCHEMA_NAME +"`.`" + TABLE_NAME + "`(size_year, unsize_year)VALUES(?,?)";
				...
				for(int i = 0; i < testData.length; i++){
					...
					Assert.assertEquals(testData[i][0], String.valueOf(rowChangedEvent.getColumns().get("size_year").getNewValue()));
					Assert.assertEquals(null, rowChangedEvent.getColumns().get("size_year").getOldValue());
					Assert.assertEquals(testData[i][0], String.valueOf(rowChangedEvent.getColumns().get("unsize_year").getNewValue()));
