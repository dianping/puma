 ...
 *
 */
public class IntTypeDebug extends AbstractBaseDebug {
			...
			public void doLogic() throws Exception {
				...
				for (int i = 0; i < testData.length; i++) {
					String insert_SQL = "INSERT INTO `" + SCHEMA_NAME + "`.`" + TABLE_NAME
					      + "`(unsigned_int, signed_int, zerofill_int, unzerofill_int)VALUES(?, ?, ?, ?)";
				...
				for (int i = 0; i < testData.length; i++) {
					...
					Assert.assertEquals(testData[i][0],
					      Integer.valueOf(String.valueOf(rowChangedEvent.getColumns().get("unsigned_int").getNewValue()))
					            .intValue());
					Assert.assertEquals(null, rowChangedEvent.getColumns().get("unsigned_int").getOldValue());
					Assert.assertEquals(testData[i][1],
					      Integer.valueOf(String.valueOf(rowChangedEvent.getColumns().get("signed_int").getNewValue()))
					            .intValue());
					Assert.assertEquals(null, rowChangedEvent.getColumns().get("signed_int").getOldValue());
					Assert.assertEquals(testData[i][2],
					      Integer.valueOf(String.valueOf(rowChangedEvent.getColumns().get("zerofill_int").getNewValue()))
					            .intValue());
					Assert.assertEquals(null, rowChangedEvent.getColumns().get("zerofill_int").getOldValue());
					Assert.assertEquals(testData[i][3],
					      Integer.valueOf(String.valueOf(rowChangedEvent.getColumns().get("unzerofill_int").getNewValue()))
					            .intValue());
	...
	public void intTypeUpdateTest() throws Exception {
		test(new TestLogic() {
			...
			public void doLogic() throws Exception {
				...
				for (int i = 0; i < testData.length; i++) {
					String update_SQL = "UPDATE `" + SCHEMA_NAME + "`.`" + TABLE_NAME
					      + "` SET unsigned_int = ?, signed_int = ?, zerofill_int = ?, unzerofill_int = ? WHERE id = ?";
				...
				for (int i = 0; i < testDataOld.length; i++) {
					...
					Assert.assertEquals(testDataOld[i][0],
					      Integer.valueOf(String.valueOf(rowChangedEvent.getColumns().get("unsigned_int").getOldValue()))
					            .intValue());
					Assert.assertEquals(testDataNew[i][1],
					      Integer.valueOf(String.valueOf(rowChangedEvent.getColumns().get("signed_int").getNewValue()))
					            .intValue());
					Assert.assertEquals(testDataOld[i][1],
					      Integer.valueOf(String.valueOf(rowChangedEvent.getColumns().get("signed_int").getOldValue()))
					            .intValue());
					Assert.assertEquals(testDataNew[i][2],
					      Integer.valueOf(String.valueOf(rowChangedEvent.getColumns().get("zerofill_int").getNewValue()))
					            .intValue());
					Assert.assertEquals(testDataOld[i][2],
					      Integer.valueOf(String.valueOf(rowChangedEvent.getColumns().get("zerofill_int").getOldValue()))
					            .intValue());
					Assert.assertEquals(testDataNew[i][3],
					      Integer.valueOf(String.valueOf(rowChangedEvent.getColumns().get("unzerofill_int").getNewValue()))
					            .intValue());
					Assert.assertEquals(testDataOld[i][3],
					      Integer.valueOf(String.valueOf(rowChangedEvent.getColumns().get("unzerofill_int").getOldValue()))
					            .intValue());
				}
			}
