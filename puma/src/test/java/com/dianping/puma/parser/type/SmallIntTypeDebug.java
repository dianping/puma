...
public class SmallIntTypeDebug extends AbstractBaseDebug {
			...
			public void doLogic() throws Exception {
				...
				for (int i = 0; i < testData.length; i++) {
					...
					Assert.assertEquals(testData[i][0],
					      Short.valueOf(String.valueOf(rowChangedEvent.getColumns().get("unsigned_smallInt").getNewValue()))
					            .shortValue());
					Assert.assertEquals(null, rowChangedEvent.getColumns().get("unsigned_smallInt").getOldValue());
					Assert.assertEquals(testData[i][1],
					      Short.valueOf(String.valueOf(rowChangedEvent.getColumns().get("signed_smallInt").getNewValue()))
					            .shortValue());
					Assert.assertEquals(null, rowChangedEvent.getColumns().get("signed_smallInt").getOldValue());
					Assert.assertEquals(testData[i][2],
					      Short.valueOf(String.valueOf(rowChangedEvent.getColumns().get("zerofill_smallInt").getNewValue()))
					            .shortValue());
					Assert.assertEquals(null, rowChangedEvent.getColumns().get("zerofill_smallInt").getOldValue());
					Assert.assertEquals(
					      testData[i][3],
					      Short.valueOf(
					            String.valueOf(rowChangedEvent.getColumns().get("unzerofill_smallInt").getNewValue()))
					            .shortValue());
		...
		test(new TestLogic() {
			...
			public void doLogic() throws Exception {
				short[][] testDataOld = { { 11, 11, 1, 1 }, { 11, -11, 9, 9 }, { 33, 77, 10, 10 }, { 33, -77, 99, 99 },
				      { 18866, 32767, 100, 100 }, { 18866, -32767, 32767, 32767 } };
				short[][] testDataNew = { { 11, -11, 1, 1 }, { 11, -11, 1, 1 }, { 33, -77, 10, 10 }, { 33, -77, 10, 10 },
				      { 18866, -32767, 100, 100 }, { 18866, -32767, 100, 100 } };
				...
				for (int i = 0; i < testDataOld.length; i++) {
					...
					Assert.assertEquals(testDataOld[i][0],
					      Short.valueOf(String.valueOf(rowChangedEvent.getColumns().get("unsigned_smallInt").getOldValue()))
					            .shortValue());
					Assert.assertEquals(testDataNew[i][1],
					      Short.valueOf(String.valueOf(rowChangedEvent.getColumns().get("signed_smallInt").getNewValue()))
					            .shortValue());
					Assert.assertEquals(testDataOld[i][1],
					      Short.valueOf(String.valueOf(rowChangedEvent.getColumns().get("signed_smallInt").getOldValue()))
					            .shortValue());
					Assert.assertEquals(testDataNew[i][2],
					      Short.valueOf(String.valueOf(rowChangedEvent.getColumns().get("zerofill_smallInt").getNewValue()))
					            .shortValue());
					Assert.assertEquals(testDataOld[i][2],
					      Short.valueOf(String.valueOf(rowChangedEvent.getColumns().get("zerofill_smallInt").getOldValue()))
					            .shortValue());
					Assert.assertEquals(
					      testDataNew[i][3],
					      Short.valueOf(
					...
					Assert.assertEquals(
					      testDataOld[i][3],
					      Short.valueOf(
					            String.valueOf(rowChangedEvent.getColumns().get("unzerofill_smallInt").getOldValue()))
					            .shortValue());
				}
			}
	...
	public void smallIntTypeDeleteTest() throws Exception {
		test(new TestLogic() {
			...
			@Override
			public void doLogic() throws Exception {
				short[][] testDataOld = { { 11, -11, 1, 1 }, { 11, -11, 1, 1 }, { 33, -77, 10, 10 }, { 33, -77, 10, 10 },
				      { 18866, -32767, 100, 100 }, { 18866, -32767, 100, 100 } };
