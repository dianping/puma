...
public class VarBinaryTypeDebug extends AbstractBaseDebug {
	...
	public static void doBefore() throws Exception {
		String create_SQL = "CREATE TABLE IF NOT EXISTS `" + SCHEMA_NAME + "`.`" + TABLE_NAME + "` (\n"
		      + "`id` int NOT NULL AUTO_INCREMENT, \n" + "`size_varBinary` varbinary(5) DEFAULT NULL, \n"
		...
		test(new TestLogic() {
			...
			public void doLogic() throws Exception {
				...
				for (int i = 0; i < testData.length; i++) {
					String insert_SQL = "INSERT INTO `" + SCHEMA_NAME + "`.`" + TABLE_NAME + "`(size_varBinary)VALUES(?)";
				...
				for (int i = 0; i < testData.length; i++) {
					...
					Assert.assertEquals(testData[i][0], rowChangedEvent.getColumns().get("size_varBinary").getNewValue());
					Assert.assertEquals(null, rowChangedEvent.getColumns().get("size_varBinary").getOldValue());
				}
			}
