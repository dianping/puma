...
public class LocalBucketTest {
	...
	public void testAppend() {
		...
		try {
			...
			Assert.assertEquals(data.length + this.localFileBucket.getStartingSequece().longValue(), localFileBucket
			      .getCurrentWritingSeq().longValue());
	...
	public void testGetNextFromZipFile() throws IOException {
		...
		com.dianping.puma.storage.oldbucket.LocalFileDataBucketManager bucketIndex = new com.dianping.puma.storage.oldbucket.LocalFileDataBucketManager();
		...
		try {
			...
			bucketIndex.copyFromLocal(new File(System.getProperty("java.io.tmpdir", "."), "Puma/").getAbsolutePath(),
			      "20120710/bucket-0");
			assertequalByteArray(datas, new LocalFileDataBucket(zipWork, new Sequence(120710, 0, 0, 0), 10000, "ffff",
			      true).getNext());
	...
	public void testSeekZipFile() throws IOException {
		...
		try {
			...
			try {
				bucketIndex.copyFromLocal(new File(System.getProperty("java.io.tmpdir", "."), "Puma/").getAbsolutePath(),
				      "20120710/bucket-0");
				LocalFileDataBucket zipBucket = new LocalFileDataBucket(zipWork, new Sequence(120710, 0, 0, 0), 10000,
				      "ffff", true);
